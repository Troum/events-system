...
export function useTrips() {
  ...
  const getTrips = async (params?: { event_slug?: string; event_id?: number; page?: number; per_page?: number }) => {
    const queryParams = new URLSearchParams()
    if (params?.event_slug) queryParams.append('event_slug', params.event_slug)
    // Обратная совместимость: поддерживаем event_id
